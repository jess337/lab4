...
def save_to_json():
    file_path = os.path.join(settings.STATIC_ROOT, 'health_data.json')
    health_data_list = list(HealthData.objects.all().values('name', 'age', 'weight', 'height', 'blood_pressure', 'cholesterol'))
    with open(file_path, 'w') as file:
        json.dump(health_data_list, file, indent=4)
